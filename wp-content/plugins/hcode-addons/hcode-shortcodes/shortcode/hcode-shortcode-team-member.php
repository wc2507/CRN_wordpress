	                ...
	                $output .= '<div class="key-person-details bg-white">';
	                    ...
	                    $output .= '<div class="person-social">';
	                		if( $hcode_team_member_fb ):
		                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_fb_url ).'" class="black-text-link"><i class="fa fa-facebook"></i></a>';
		                	endif;
		                	if( $hcode_team_member_tw ):
		                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_tw_url ).'" class="black-text-link"><i class="fa fa-twitter"></i></a>';
		                	endif;
		                	if( $hcode_team_member_googleplus ):
		                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_googleplus_url ).'" class="black-text-link"><i class="fa fa-google-plus"></i></a>';
		                	endif;
	                		if( $hcode_team_member_db ):
		                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_db_url ).'" class="black-text-link"><i class="fa fa-dribbble"></i></a>';
		                	endif;
	                		if( $hcode_team_member_yt ):
		                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_yt_url ).'" class="black-text-link"><i class="fa fa-youtube"></i></a>';
		                	endif;
	                		if( $hcode_team_member_li ):
		                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_li_url ).'" class="black-text-link"><i class="fa fa-linkedin"></i></a>';
		                	endif;
	                		if( $hcode_team_member_ig ):
		                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_ig_url ).'" class="black-text-link"><i class="fa fa-instagram"></i></a>';
		                	endif;
	                		if( $hcode_team_member_pi ):
		                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_pi_url ).'" class="black-text-link"><i class="fa fa-pinterest-p"></i></a>';
		                	endif;
	                		if( $hcode_team_member_gh ):
		                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_gh_url ).'" class="black-text-link"><i class="fa fa-github"></i></a>';
		                	endif;
	                		if( $hcode_team_member_ws ):
		                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_ws_url ).'" class="black-text-link"><i class="fa fa-external-link"></i></a>';
		                	endif;
		                ...
		                $output .= '<div class="team-details bg-blck-overlay">';
		                	...
		                	if( $hcode_team_member_fb || $hcode_team_member_tw || $hcode_team_member_googleplus
			                    ...
			                    $output .= '<div class="person-social margin-five">';
	                                            if( $hcode_team_member_fb ):
	                                            $output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_fb_url ).'" class="white-text-link"><i class="fa fa-facebook"></i></a>';
	                                            endif;
	                                            if( $hcode_team_member_tw ):
	                                            $output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_tw_url ).'" class="white-text-link"><i class="fa fa-twitter"></i></a>';
	                                            endif;
	                                            if( $hcode_team_member_googleplus ):
	                                            $output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_googleplus_url ).'" class="white-text-link"><i class="fa fa-google-plus"></i></a>';
	                                            endif;
	                                            if( $hcode_team_member_db ):
	                                            $output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_db_url ).'" class="white-text-link"><i class="fa fa-dribbble"></i></a>';
	                                            endif;
						                		if( $hcode_team_member_yt ):
							                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_yt_url ).'" class="white-text-link"><i class="fa fa-youtube"></i></a>';
							                	endif;
						                		if( $hcode_team_member_li ):
							                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_li_url ).'" class="white-text-link"><i class="fa fa-linkedin"></i></a>';
							                	endif;
						                		if( $hcode_team_member_ig ):
							                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_ig_url ).'" class="white-text-link"><i class="fa fa-instagram"></i></a>';
							                	endif;
						                		if( $hcode_team_member_pi ):
							                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_pi_url ).'" class="white-text-link"><i class="fa fa-pinterest-p"></i></a>';
							                	endif;
						                		if( $hcode_team_member_gh ):
							                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_gh_url ).'" class="white-text-link"><i class="fa fa-github"></i></a>';
							                	endif;
						                		if( $hcode_team_member_ws ):
							                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_ws_url ).'" class="white-text-link"><i class="fa fa-external-link"></i></a>';
							                	endif;
							                	if( !empty( $hcode_team_member_custom_link ) ) :
							                		$output .= nl2br( rawurldecode( base64_decode( strip_tags( $hcode_team_member_custom_link ) ) ) );
							                	endif;
						...
						$output .= '<div class="architecture-team team-member xs-no-padding">';
		                    ...
		                    if( $hcode_team_member_fb || $hcode_team_member_tw || $hcode_team_member_googleplus
			                    ...
			                    $output .= '<div class="person-social margin-ten xs-margin-0auto">';
			                		if( $hcode_team_member_fb ):
				                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_fb_url ).'" class="black-text-link"><i class="fa fa-facebook no-margin-left"></i></a>';
				                	endif;
				                	if( $hcode_team_member_tw ):
				                    	$output .= '<a '.$target.' href="'.esc_url( $hcode_team_member_tw_url ).'" class="black-text-link"><i class="fa fa-twitter"></i></a>';
				                	endif;
