...
if ( ! function_exists( 'hcode_row' ) ) {
	function hcode_row($atts, $content = null){
		...
		$output = $padding = $padding_style = $margin = $margin_style = $style_attr = '';
		...
		$equal_height = ( $equal_height ) ? ' row-equal-height' : '';
		$content_placement = ( $content_placement ) ? ' row-content-' . $content_placement : '';
		...
		if(empty($hcode_row_style)){
			...
			if(!empty($hcode_row_section_class) || !empty($position_relative) || !empty($overflow_hidden) || !empty($hcode_relative_projects) || !empty($padding) || !empty($fullscreen) || !empty($equal_height) || !empty($content_placement) ){
				$hcode_row_section_class_att  .= 'class="'.$css_class.$hcode_row_section_class.$equal_height.$content_placement.$fullscreen.$position_relative.$overflow_hidden.$hcode_relative_projects.$scroll_to_down.$scroll_to_down_class.$padding.'"';
			}
			$output .= '<section'.$id.' '.$hcode_row_section_class_att.' '.$hcode_row_section_style.$scroll_to_down_onclick.'>';
			...
			switch ($hcode_row_style) {
				case 'color':
					...
					$output .= '<section'.$id.' class="'.$css_class.$class.$equal_height.$content_placement.$fullscreen.$hcode_row_padding_class.$hcode_row_mobile_padding.$hcode_row_animation_style_att.$position_relative.$overflow_hidden.$scroll_to_down.$scroll_to_down_class.$margin.$padding.'" '.$hcode_row_section_style.$scroll_to_down_onclick.'>';
						if($show_full_width == '1'){
							$output .=  do_shortcode($content);
						}else{
		                	$output .= '<div class="'.$show_container_fluid_att.'">';
				...
				case 'image':
					...
					$output .= '<section '.$id.' class="'.$css_class.$class.$equal_height.$content_placement.$hcode_row_padding_class.$hcode_row_mobile_padding.$hcode_row_image_type_att.$hcode_parallax_image_type_att.$hcode_bg_image_type_att.$hcode_row_animation_style_att.$fullscreen.$position_relative.$overflow_hidden.$scroll_to_down.$hide_background.$scroll_to_down_class.$margin.$padding.'" '.$hcode_row_section_style.$scroll_to_down_onclick.'>';
...
add_shortcode( 'vc_section', 'hcode_row' );
?>