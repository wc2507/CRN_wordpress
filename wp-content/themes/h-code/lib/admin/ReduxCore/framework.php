...
//                }
            ...
            public function _options_page_html() {
                ...
                echo '<div class="hcode-head-right">';
                    echo '<a href="http://wpdemos.themezaa.com/h-code/documentation/" target="_blank">'.__('Online Documentation', 'H-Code').'</a>';
                    echo '<span class="link_sep">|</span><a href="http://www.themezaa.com/support.php" target="_blank">'.__('Support Center', 'H-Code').'</a>';
                echo '</div>';
                echo '<div class="clear"></div>';
                echo '</div>';
                ...
                if ( isset( $this->transients['last_save_mode'] ) ) {
                    ...
                    //echo $this->transients['last_save_mode'];die;
                    unset( $this->transients['last_save_mode'] );
                    //$this->transients['last_save_mode'] = 'remove';
                    $this->set_transients();
                }
                echo '<div class="saved_notice admin-notice notice-green" style="display: none;"><strong>' . apply_filters( "redux-saved-text-{$this->args['opt_name']}", __( 'Settings Saved!', 'redux-framework' ) ) . '</strong></div>';
                /**
                ...
                if ( !empty( $section['tab'] ) ) {
                    ...
                    */
                    ...
                    if ( $display ) {
                       $this->do_settings_sections( $this->args['opt_name'] . $k . '_section_group' );
                    }
                    //}
                    echo "</div>";
                    //echo '</div>';
                ...
                echo '<div id="redux-footer-sticky" style="display: none;"><div id="redux-footer">';
            ...
            */
            public function do_settings_sections( $page ) {
                ...
                foreach ( (array) $wp_settings_sections[$page] as $section ) {
                    ...
                    // Change For H-Code Accordion Tab
                    echo '<div class="form-table">';
                    if( class_exists( 'WooCommerce' ) ){
                        if( $section['id'] == 'hcode_theme_setting13_section' ){
                            do_settings_fields( $page, $section['id'] );
                        }else{
                            $this->do_settings_fields( $page, $section['id'] );
                        }
                    }else{
                        if( $section['id'] == 'hcode_theme_setting12_section' ){
                            do_settings_fields( $page, $section['id'] );
                        }else{
                            $this->do_settings_fields( $page, $section['id'] );
                        }
            ...
            public function do_settings_fields($page, $section) {
                ...
                foreach ( (array) $wp_settings_fields[$page][$section] as $field ) {
                    ...
                    if(isset($field['args']['type']) && $field['args']['type'] == 'accordion' && $field['args']['position'] == 'start'){
                        ...
                        echo '</div>';
                        echo '<div class="hcode-admin-tab-slidingdiv">';
                    }

                    if(isset($field['args']['type']) && $field['args']['type'] == 'info_title' && $field['args']['title'] != ''){
                        echo '<div class="hcode-fileds-row">';
                            echo '<div class="hcode-info-title" id="'.$field['args']['id'].'">'.$field['args']['title'];
                            if(isset($field['args']['subtitle'])){
                                echo '<div class="subtitle">'.$field['args']['subtitle'].'</div>';
                            }
                            echo '</div>';
                        echo '</div>';
                    }

                    $array_type = array('accordion', 'info_title');
                    if (!in_array($field['args']['type'], $array_type)) {
                        echo '<div class="hcode-fileds-row">';
                            if ( !empty($field['args']['label_for']) )
                                ...
                                echo '<div scope="row" class="hcode-admin-title">' . $field['title'] . '</div>';
                            echo '<div class="hcode-td">';
