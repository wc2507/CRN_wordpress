    ...
    switch ($hcode_blog_page_layout) {
        case 'grid':
            ...
            $image_title = ( isset($img_title['title']) && !empty($img_title['title']) ) ? $img_title['title'] : '';
                ...
                if( $hcode_show_post_author == 1 ) {
                    $posted_by[] = esc_html__('Posted by ','H-Code'). '<a href='.get_author_posts_url( get_the_author_meta( 'ID' ) ).'>'.get_the_author().'</a>';
                }
                if( $hcode_show_post_meta == 1 ) {
                    $posted_by[] = get_the_date( $hcode_date_format, get_the_ID());
                }
        ...
        case 'modern':
               $i = 1;
                ...
                $show_excerpt = ( $hcode_show_excerpt == 1 ) ? wpautop(hcode_get_the_excerpt_theme($hcode_excerpt_length)) : '';
                $hcode_show_author = ( $hcode_show_post_author == 1 ) ? '<div class="blog-date-right light-gray-text2">'.esc_html__('Posted by ','H-Code'). '<a href='.get_author_posts_url( get_the_author_meta( 'ID' ) ).'>'.get_the_author().'</a></div><div class="separator-line bg-black no-margin-lr no-margin xs-margin-ten-bottom"></div>' : '';
